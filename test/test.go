package test

import (
	"github.com/Mishka-Squat/gamemath/rect2"
	"github.com/Mishka-Squat/gamemath/vector2"
	"github.com/Mishka-Squat/gamemath/vector3"
	"github.com/Mishka-Squat/goex/mathex"
	"github.com/stretchr/testify/assert"
)

func AssertVector2InDelta[T mathex.SignedNumber](t assert.TestingT, expected, actual vector2.Of[T], delta float64) {
	assert.InDelta(t, expected.X, actual.X, delta)
	assert.InDelta(t, expected.Y, actual.Y, delta)
}

func AssertVector3InDelta[T mathex.SignedNumber](t assert.TestingT, expected, actual vector3.Of[T], delta float64) {
	assert.InDelta(t, expected.X, actual.X, delta)
	assert.InDelta(t, expected.Y, actual.Y, delta)
	assert.InDelta(t, expected.Z, actual.Z, delta)
}

func AssertRectangle2InDelta[T mathex.SignedNumber](t assert.TestingT, expected, actual rect2.Of[T], delta float64) {
	AssertVector2InDelta(t, expected.Position, actual.Position, delta)
	AssertVector2InDelta(t, expected.Size, actual.Size, delta)
}
